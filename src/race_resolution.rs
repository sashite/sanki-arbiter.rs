...
//!
//! All authoritative timing is anchored on the designated timestamper's Event
//! Timestamp Attestations (kind `1041`); a suite event's own `created_at` is
//! informational and never consulted here. Two selection primitives are needed
//! for per-ply adjudication:
//!
//! - **Meta-resolution** ([`canonical_attestation`]) — when an event has more
//!   than one timestamper attestation, the canonical one has the smallest
//!   `created_at`, ties broken by the smallest attestation event id.
//! - **Slot selection** ([`canonical_ply`]) — among the Plies competing for one
//!   `(session, signer, step)` slot, the canonical one is the Ply whose
//!   canonical attestation has the smallest `created_at`, ties broken by the
//!   smallest *Ply* event id. A Ply without a conforming timestamper attestation
//!   has no canonical timing and is excluded (it is *pending*).
//!
...
/// timing — the canonical attestation's `created_at`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CanonicalPly<'a> {
    /// The canonical Ply.
    pub ply: &'a Ply,
    /// The authoritative timing of the Ply (its canonical attestation's
    /// `created_at`).
    pub at: Timestamp,
}
...
/// candidate's authoritative timing is its canonical attestation's `created_at`
/// (per [`canonical_attestation`]); candidates without one are pending and
/// excluded. The canonical Ply has the smallest such timing, ties broken by the
/// smallest Ply event id. `None` if no candidate is attested.
#[must_use]
pub fn canonical_ply<'a>(
    ...
    candidates
        .into_iter()
        .filter_map(|ply| {
            canonical_attestation(attestations, ply.id, timestamper).map(|attestation| {
                CanonicalPly {
                    ply,
...
mod tests {
    ...
    use super::{canonical_attestation, canonical_ply};
    use crate::event::{Attestation, EventId, Ply, PublicKey};
    use sashite_sanki_engine::domain::time::Timestamp;
    ...
    fn ply(id: u8, signer: u8, step: u32) -> Ply {
        Ply::new(eid(id), pk(signer), eid(200), step, false, String::new())
    }
    ...
    fn slot_smallest_attestation_created_at() {
        ...
        let canonical = canonical_ply(plies.iter(), &atts, pk(TIMESTAMPER)).expect("a candidate");
    ...
    fn slot_no_attested_ply_yields_none() {
        ...
        assert!(canonical_ply(plies.iter(), &atts, pk(TIMESTAMPER)).is_none());
    }
}
